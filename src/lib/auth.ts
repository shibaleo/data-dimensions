/**
 * Auth for data-dimensions.
 * Supports: API Key (ddm_ prefix) and Clerk JWT (email verified against DB).
 */
...
async function verifyApiKeyToken(token: string): Promise<AuthResult | null> {
  ...
  const prefix = token.slice(0, 12); // ddm_ + 8 chars
  const rows = await db
    .select()
    .from(apiKey)
  ...
  const rawKey = token.slice(4); // strip ddm_ prefix
